...
section~\ref{appli}. Finally, we study the problem of throwing a needle on a 
finite table.
...
A graph $(V,E) $ is said to be \emph{unit distance} if it has a 
...
same colours when colouring the graph $g$ with $n$ colours, divided by the 
...
\end{proof}
We still don't know wether or not this is actually an equality. For $n=2$, it 
is an equality, as will be shown thereafter.
