...
\begin{abstract} If you colour a table using $k$ colours, and throw a needle
randomly on it, you get a certain probability that the endpoints will fall on 
...
A well-known problem of geometric graph theory is the following: how many colours
are needed to colour the plane so that no two points at unit distance are the 
same colour? This is known as the Hadwiger-Nelson problem, and the answer to that
...
problem, since we don't try to make \textit{every} unit-length segment 
bi-chromatic but a large fraction of them.

\medskip

Let's state our results in a rough, imprecise way: a $k$-\textit{colouring} is a borelian 
...
the latter being a generalisation of the former.
\begin{definition}
\
\begin{enumerate}[i)] 
\item A $k$-\textit{colouring} of the plane is a measurable function $c$ from the euclidean 
...
\[ \M (c) = \{x,y \in \R ^2 \times \R ^2 \mid d(x,y) = 1, \ c(x) = c(y) \} \]
...
case we note $\P_c$ the parallelogram formed on $u$ and $v$, that is:
\[\P_c = \{tu+sv, \ 0\leq t,s< 1 \} \subset \R ^2. \]
\item For a periodic colouring $c$, consider the following random variables:
...
let $M_G(c)$ be the number of monochromatic edges in $G$ when coloured with $c$:
...
have~:\\
 ...
 In particular, the probability does not depend on the graph 
 chosen.
\end{lemma}
...
With two colours, we consider an equilateral triangle as a unit distance graph. 
As there are always at least two vertices with the same colour, it's clear that 
...
three vertices with the same colours. Applying Lemma~\ref{huitre}, we conclude 
that this colouring achieves a probability of $\frac{1}{3}$ indeed.
%CLASSE DE SOLUTIONS?

\subsection{3 colours}
 With three colours, we use the Moser spindle of figure~\ref{colour} - a unit 
...
\caption{\label{colour} The Moser spindle}
\end{figure}
...
this colouring gives a $\pper(c)$ of about $0.13$ for an edge of about 
...
$k$-colouring is always greater than a certain positive constant. Taking the 
...
Let $c$ be a $k$-colouring of $\mathbf{P}$. Let $\tilde{c}$ be the periodic version of
...
If $c$ is an asymptotic colouring, let $\tilde{c_R}$ be the periodic version of $c_{|[-R;R]^2}$ (that is, restrict $c$ to $[-R;R]^2$ and tile the plane by repeating this square). By lemma~\ref{choco},
...
However, it should be noted that the value of $p(c)$ defined by asymptotic colourings on different shapes may differ. For instance, consider the 2-colouring of figure \ref{contrex} where stripes are $\frac{\sqrt{3}}{2}$ apart. In the striped region the probability is approximately $\frac13$ (see section \ref{2col}), and it is $1$ in the rest of the plane. So $p(c)$ is related to the fraction of a square (resp. disk) occupied by the striped region, and this fraction depends on the shape.
...
%For instance, in dimension $d$ with $2$ colours,
