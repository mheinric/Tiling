...
some bounds over the probability.p\end{abstract}
...
the colouring is \textit{periodic} - that is, there is a basis of two vectors, $u$ and $v$, such that translations along these vectors leave the colouring unchanged. Thereafter, we will always assume that the colouring is measurable and periodic, and we will denote $\mathbf{P}$ the parallelogram defined by these vectors. Needle throwing processes will be considered on this parallelogram only.
