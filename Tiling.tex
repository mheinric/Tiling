...
consequences, and try to generalise this idea in higher dimensions in section 
\ref{appli}. Finally, we study the problem of launching a needle on a finite 
table.
...
We remark that in higher dimension, the bounds may change : for
example, with $3$ colours in dimension $3$, the regular tetrahedron witnesses a 
better lower bound
...
$n$-colouring is always greater than a certain constant. Taking the 
...
Once again, let's focus on the first term. Let $\mathcal{P'}$ be the 
...
easily shows that we get the same main results for $p$, especially consequence 
\ref{ineg} when taking $C_n$ the set of acceptable colourings with $n$ colours. 
