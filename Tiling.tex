...
is a coloring, let $p(c)$ be the probability that when throwing a unit-length
segment (or \textit{needle}) on $c$, you get both endpoints on the same color.
...
edges in a $k$-coloring of $g$. Then, if $\mathcal{G}$ denotes the set of 
\textit{unit distance graphs} -- that is, graphs that can be embedded in the
plane with all edges of length 1 -- and $\C_k$ the set of $k$-colorings, we
have:
\begin{theo*}
...
A \textit{coloring} is a $k$-coloring for some $k$.
\item The set of \textit{monochromatic needles} of a coloring $c$ is the set
...
\item If $A$ is a measurable subset of $\R^2$ and $c$ a coloring, let $\N_A$ be
...
let $M_G(c)$ be the number of monochromatic edges in $G$ when colored with $c$:
$$M_G(c) = \sum_{(i, j) \in E} \mathds{1}_{c(i)=c(j)}.$$
where the sum is on the edges of $G$.

Then we define $m_k(G)$ as:
\[m_k(G) = \min_{c \ k-\mathrm{coloring \ of} \ G} \frac{M_G(c)}{|E|}.\]
...
Let $c$ be a periodic coloring, and consider the needle-throwing ``processes'':
...
this coloring gives a $\pper(c)$ of about $0.13$ for an edge of about 
