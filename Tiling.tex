...
\begin{proof}
Notice that there is a one-to-one correspondance between needles given by 
Process~\ref{premier} and points of $E := \mathbf{P} \times [0;\pi ]$, moreover, the 
law of the needle is exactly the normalised Lebesgue measure on the set $E$.

We still define $c$ outside of $\mathbf{P}$ by using periodicity. Now consider 
the following subsets~:
\begin{eqnarray*}
E' & = & \{ (A,\theta) \in \mathbf{P} \times [0;\pi ] \mid A+e^{i \theta} \in \mathbf{P} \}, \\
F & = & \{ (A,\theta) \in \mathbf{P} \times [0;\pi ] \mid c(A) = c(A+e^{i \theta}) \}, \\
F' & = & E' \cap F,
\end{eqnarray*}
and denote by $V$, $V'$, $W$, $W'$ the volumes of, respectively, 
$E$, $E'$, $F$, $F'$. Then we simply have $p(c) = \frac{W}{V}$ and 
$p'(c) = \frac{W'}{V'} $.

When the point $A$'s distance to the border is more than $1$, B has to fall in 
$\mathbf{P}$, which implies that
$E \setminus E' \subsetneq K(\mathbf{P},1) \times [0;\pi[$.
There is therefore a number $t \in ]0;1[$ so that 
$$V-V' = u \pi \mathcal{A} (K(\mathbf{P},1)). $$
Similarly, there is a number $u \in [0;1[$ so that 
$$W-W' = t \pi \mathcal{A} (K(\mathbf{P},1)). $$

Then, by direct computation, 
\begin{eqnarray*}
p'(c) - p(c) & = & \frac{W'}{V'} - \frac{W}{V} \\
& = & \frac{W}{V} \frac{V-V'}{V'} - \frac{W-W'}{V'} \\
& = & p(c) \frac{t \mathcal{A} (K(\mathbf{P},1))}{\mathcal{A} (\mathbf{P}) - t \mathcal{A} (K(\mathbf{P},1))} - \frac{u \mathcal{A} (K(\mathbf{P},1))}{\mathcal{A} (\mathbf{P}) - t \mathcal{A} (K(\mathbf{P},1))}.
\end{eqnarray*}
If we denote $r = \frac{\mathcal{A} (K(\mathbf{P},1))}{\mathcal{A} (\mathbf{P})} \leq 1 $, 
we get
$$p'(c) - p(c) = p(c) \frac{tr}{1-tr} - \frac{ur}{1-tr},$$
so that
\begin{eqnarray*}
|p'(c) - p(c)| & = & \frac{r}{1-tr} |t p(c) - u| \\
& \leq & \frac{1}{1-tr} r
\end{eqnarray*}
because $t p(c)$ and $u$ are in $[0;1]$.

Now we set $\kappa = 2$. Indeed, when $r \leq \frac12$ the previous inequality 
implies $|p'(c) - p(c)| \leq 2r$, and when $r > \frac12$ this is obvious.
\end{proof}
