...
\item Choose an angle $\theta$ in $[0 ; \pi[$ such that the second end 
...
\begin{process}$G$ denotes a unit distance graph, and $r=\partial (G,f)$ for some 
unit legth representation $f$ of $G$.
\begin{itemize}
  \item let $\theta '$ (the orientation of our graph) be chosen uniformly 
  in $[0; 2 \pi[$ ;
  \item let $A_0$ be a point chosen uniformly in $\mathbf{P} \setminus K(\mathbf{P},r)$ ;
  \item let $i$ be an index chosen uniformly in $[|1;m|]$.
\end{itemize}
